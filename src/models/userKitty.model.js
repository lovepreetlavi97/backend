...
userKittySchema.pre('save', function(next) {
  ...
  if (this.isNew && !this.nextPaymentDate) {
    ...
    this.nextPaymentDate.setDate(this.nextPaymentDate.getDate() + 30);
  }
...
userKittySchema.methods.generateNextPayment = function() {
  ...
  if (!lastPayment) {
    ...
    return {
      ...
      dueDate: this.nextPaymentDate,
      status: 'pending'
    };
  }
  
  // Next payment after 30 days
  const nextDueDate = new Date(lastPayment.paymentDate);
