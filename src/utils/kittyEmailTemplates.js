...
const getKittyCompletionTemplate = (data) => {
  const { userName, planName, totalPaid, totalAmount, maturityAmount, date } = data;
...
<head>
  ...
  <style>
    ...
    .greeting { font-family: 'Playfair Display', serif; font-size: 24px; margin-bottom: 24px; }
    .success-badge { display: inline-block; padding: 8px 16px; background: #f0fdf4; color: #166534; border-radius: 100px; font-size: 10px; font-weight: 900; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 30px; border: 1px solid #dcfce7; }
    .amount-card { background: #1a1a1a; border-radius: 24px; padding: 30px; margin-bottom: 40px; text-align: center; color: #ffffff; }
    .amount-label { font-size: 10px; font-weight: 900; text-transform: uppercase; letter-spacing: 2px; color: #c97f5e; margin-bottom: 8px; }
    .amount-value { font-family: 'Playfair Display', serif; font-size: 36px; color: #ffffff; }
    .details-grid { display: grid; grid-template-cols: 1fr 1fr; gap: 24px; margin-bottom: 40px; border-top: 1px solid #f0f0f0; padding-top: 40px; }
    .detail-item { }
    .detail-label { font-size: 9px; font-weight: 900; text-transform: uppercase; letter-spacing: 1px; color: #a1a1a1; margin-bottom: 4px; }
    .detail-value { font-size: 14px; font-weight: 700; color: #1a1a1a; }
    .footer { padding: 40px; text-align: center; font-size: 12px; color: #a1a1a1; border-top: 1px solid #f0f0f0; }
    .cta { display: inline-block; padding: 16px 32px; background: #c97f5e; color: #ffffff !important; text-decoration: none; border-radius: 16px; font-size: 12px; font-weight: 900; text-transform: uppercase; letter-spacing: 2px; margin-top: 20px; }
  </style>
</head>
<body>
  <div class="container">
    ...
    <div class="content">
      <div class="success-badge">Plan Completed</div>
      <div class="greeting">Congratulations, ${userName}</div>
      <p style="margin-bottom: 40px; color: #666;">Your kitty plan <strong>${planName}</strong> has been successfully completed. Your savings are now ready for redemption.</p>
      
      <div class="amount-card">
        <div class="amount-label">Total Maturity Value</div>
        <div class="amount-value">₹${maturityAmount.toLocaleString()}</div>
      </div>
<br/>
      <div class="details-grid">
        <div class="detail-item">
        ...
        <div class="detail-item">
          ...
          <div class="detail-value">₹${totalPaid.toLocaleString()}</div>
        </div>
        <div class="detail-item">
          <div class="detail-label">Bonus Value</div>
          <div class="detail-value">₹${(maturityAmount - totalPaid).toLocaleString()}</div>
        </div>
        <div class="detail-item">
          <div class="detail-label">Status</div>
          <div class="detail-value" style="color: #166534;">READY TO REDEEM</div>
        </div>
      </div>

      <div style="text-align: center; background: #fdfbf7; padding: 30px; border-radius: 20px; border: 1px solid #f3eee1;">
        <p style="font-size: 14px; color: #666; margin-bottom: 20px;">Visit our store or use your account online to redeem your maturity value against our exquisite jewelry collections.</p>
        <a href="https://gurujewellers.in/collections/all" class="cta">Start Shopping</a>
      </div>
    </div>
    <div class="footer">
      <p>&copy; 2026 Guru Jewellers. All rights reserved.</p>
...
module.exports = { 
  getKittyPaymentSuccessTemplate,
  getKittyCompletionTemplate
};
