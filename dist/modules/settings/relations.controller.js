...
let RelationsController = class RelationsController {
    ...
    async getAllRelations(status) {
        ...
        if (status !== 'all') {
            relations = relations.filter((r) => r.isActive !== false && r.status !== 'inactive');
        }
...
__decorate([
    (0, common_1.Get)(),
    (0, swagger_1.ApiOperation)({ summary: 'Get relations/recipients (defaults to active for public)' }),
