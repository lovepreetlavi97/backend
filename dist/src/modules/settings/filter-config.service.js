...
let FilterConfigService = FilterConfigService_1 = class FilterConfigService {
    ...
    validateConfig(dto) {
        if (!dto.priceFilters || !Array.isArray(dto.priceFilters)) {
            throw new common_1.BadRequestException('priceFilters must be an array.');
        }
        const priceIds = new Set();
        for (const pf of dto.priceFilters) {
            if (pf.min >= pf.max) {
                throw new common_1.BadRequestException(`Invalid price range for '${pf.label}': min (${pf.min}) must be strictly less than max (${pf.max}).`);
            }
            if (priceIds.has(pf._id)) {
                throw new common_1.BadRequestException(`Duplicate price filter ID found: '${pf._id}'.`);
            }
            priceIds.add(pf._id);
        }
        ...
        for (const occ of dto.occasions || []) {
            if (occasionSlugs.has(occ.slug)) {
                throw new common_1.BadRequestException(`Duplicate occasion slug found: '${occ.slug}'.`);
            }
        ...
        for (const rec of dto.recipients || []) {
            if (recipientSlugs.has(rec.slug)) {
                throw new common_1.BadRequestException(`Duplicate recipient slug found: '${rec.slug}'.`);
            }
