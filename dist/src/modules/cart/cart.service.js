...
let CartService = class CartService {
    ...
    async addToCart(userId, guestId, productId, quantity = 1) {
        if (!userId && !guestId) {
            throw new common_1.NotFoundException('Either userId or guestId is required.');
        }
    ...
    async removeFromCart(cartItemId, userId, guestId) {
    ...
    async syncGuestCartToUser(guestId, userId) {
        if (!guestId || !userId)
            return;
